...
### 
    #my $string = params->{'text'};
    my $category_date = params->{'category'};
    my $id = params->{'id'};
    chomp($category_date);
    #chomp($string);
    chomp($id);
           #my $string = cleaner($string);
           my $category_date = cleaner($category_date);
           my $id = cleaner($id);
           my $db = connect_db();

           ###my $sql = "update entries set status = \'$string\' where id =\'$id\'"; 
           my $sql = "update entries set category = \'$category_date\' where id =\'$id\'"; 
           print "$sql\n";
           my $sth = $db->prepare($sql) or die $db->errstr;
...
###
### edit 585 to 913
###### DELETE 585 to 913 upt to /editdate
...
#    chomp($$category_date);
   ...
   #     print "\$item: $item\n";
         ...
         #  chomp($status);
           ...
           ###my $sql = 'update entries set status = \''.$string.'\' where id =\''.$id.'\''; 
           my $sql = "update entries set status = \'$string\' where id =\'$id\'"; 
           ###my $sql_cat_date = "update entries set category = \'$category_date\' where id =\'$id\'"; 
